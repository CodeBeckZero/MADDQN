...
class BuyHoldAgent(BaseAgent):
    """
    Buy and hold agent executes an buy and sell action at the beginning and end, respectively.    
    """
    ...
    def test(self, start_idx:int, end_idx:int, testing_episodes=1, metric_func = None, metric_func_arg = {}):
        ...
        self.tst_start_idx = start_idx
        self.tst_end_idx = end_idx
    ...
    def _act(self, step_type):
        ...
        if self.env.current_step == self.env.start_idx:
            action = 'B'
        elif self.env.current_step == self.env.finish_idx-1:
            action = 'S'
        else:
            action = 'H'
        action_type = "Manual"    
